...
function convertUnicodeToBijoy(text) {
    ...
    // ------------------------------------------------------------
    // IMPORTANT: legacy Bijoy forms that must be resolved BEFORE
    // generic consonant/kar processing.
    // ------------------------------------------------------------
    const specialJukta = [
        ["ক্ষ্ম্য", "²"],
        ["ক্ষ্ম", "²"],
        ...
        // বৈ / সৈ use the legacy Bijoy ৈ form.
        ["বৈ", "‰e"],
        ["সৈ", "‰m"],

        // Other frequently problematic conjuncts.
        ["শ্ম", "k¥"],
        ["ক্স", "·"],
        ["গ্র", "MÖ"],
        ["ঞ্চ", "Â"],
        ...
        // Common ya-phala combinations used in legacy Bijoy.
        ["ত্যু", "Zz¨"],
        ["প্যু", "cz¨"],
        ["ফ্যু", "dz¨"],
        ["ভ্যু", "fz¨"],
    ...
    // Protect special forms with private placeholders so that the
    // generic conversion engine cannot destroy them.
    const protectedTokens = [];
    specialJukta.forEach(([unicode, bijoy], index) => {
        const token = `\uE000${index}\uE001`;
        if (str.includes(unicode)) {
            str = str.split(unicode).join(token);
            protectedTokens.push([token, bijoy]);
        }
    });

    // ------------------------------------------------------------
    // Standard vowel ordering.
    // ------------------------------------------------------------
    str = str
        .replace(/ো/g, "ো")
        .replace(/ৌ/g, "ৌ");
    ...
    // ------------------------------------------------------------
    // Standard Unicode -> Bijoy conjunct mappings.
    // Longest first.
    // ------------------------------------------------------------
    const u2bJukta = {
    ...
    const keys = Object.keys(u2bJukta)
        .sort((a, b) => b.length - a.length);

    for (const key of keys) {
        str = str.split(key).join(u2bJukta[key]);
    }

    // ------------------------------------------------------------
    // Legacy special vowel combinations.
    // ------------------------------------------------------------
    str = str
        .replace(/বৈ/g, "‰e")
        .replace(/সৈ/g, "‰m")
    ...
    // ------------------------------------------------------------
    // Restore protected legacy forms.
    // ------------------------------------------------------------
    protectedTokens.forEach(([token, bijoy]) => {
        out = out.split(token).join(bijoy);
    });

    // ------------------------------------------------------------
    // Final correction layer.
    // These specifically prevent the generic engine from producing
    // the broken forms seen in the supplied test cases.
    // ------------------------------------------------------------
    const finalFixes = [
        ["¶g", "²"],
        ["k&g", "k¥"],
    ...
    for (const [wrong, correct] of converterFinalFixes) {
